...
import time
import argparse
from spower.utils import env, downloadFile, runCommand, mkdir_p, Field
...
class SKAT(RTest):
    ...
    def _determine_algorithm(self):
        ...
        self.datvar = 'dat'
        basename = 'SKAT{0}{1}'.format(time.strftime('%b%d%H%M%S', time.gmtime()), os.getpid())
        # write header
        self.Rscript.append(basename + ' <- function(dat) {')
        self.Rscript.append('m <- ncol(dat@Y)')
        self.Rscript.append('Z <- as.matrix(dat@X)')
        # clean up missing data
        if self.mafupper < 1 or self.maflower > 0:
            clean_missing = '''
            zmaf <- apply(Z, 2, function(x) sum(x[which(x == 1 | x == 2)])) / nrow(Z) / 2
            Z <- as.matrix(Z[,which(zmaf > {0} & zmaf < {1})])
            '''.format(self.maflower, self.mafupper)
            self.Rscript.extend([x.strip() for x in clean_missing.split('\n')])
